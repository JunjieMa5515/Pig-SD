...
uint16_t adc_get_result(uint32_t channel)
{
    ...
    adc_channel_set(&hadc1, channel, 1, ADC_SAMPLETIME_480CYCLES);   /* 设置ADC通道 */
    HAL_ADC_Start(&hadc1);                                           /* 开启ADC */
    HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);                /* ADC轮询转换 */
    result = HAL_ADC_GetValue(&hadc1);                               /* 获取ADC值 */
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
 ...
 adc_result = HAL_ADC_GetValue(&hadc1);
	vvv= adc_result*3.3f/4095.0f;
			  Pulling = ((adc_result * 3.3f) / 4095.0f)*4000.0f;  
